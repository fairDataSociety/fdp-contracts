...
import "../../ENS.sol";
import "./FDSRegistrarController.sol";
...
contract BulkRenewal {
    ...
    function getController() internal view returns(FDSRegistrarController) {
        Resolver r = Resolver(ens.resolver(ETH_NAMEHASH));
        return FDSRegistrarController(r.interfaceImplementer(ETH_NAMEHASH, REGISTRAR_CONTROLLER_ID));
    }

    function rentPrice(string[] calldata names, uint duration) external view returns(uint total) {
        FDSRegistrarController controller = getController();
